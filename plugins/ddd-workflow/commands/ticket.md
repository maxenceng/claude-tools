...
Otherwise dispatch on the first word — one of `new`, `start`, `review`, `respond`, `done`
— by reading `${CLAUDE_PLUGIN_ROOT}/ticket-steps/<word>.md` and following it. Read that
one file and no others: the steps do not share instructions at runtime, and loading the
four that are not running is most of what this command used to cost.
